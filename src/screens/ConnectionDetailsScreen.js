...
import * as Contacts from 'expo-contacts'; // <--- NEW IMPORT
...
export default function ConnectionDetailsScreen({ route, navigation }) {
    ...
    try {
      ...
      if (status === 'granted') {
        ...
        const contact = {
          ...
          firstName: profile.full_name || 'Knect User',
          jobTitle: profile.job_title || '',
          // You could map Socials to "Notes" or specific fields if you wanted
          note: `Met via Knect on ${new Date(connection.created_at).toDateString()}.\n\nLinkedIn: ${profile.linkedin}\nGitHub: ${profile.github}`,
          imageAvailable: !!profile.avatar_url,
        };

        // 3. Open System Contact UI (Best UX)
        const contactId = await Contacts.presentFormAsync(null, contact, {
          isNew: true,
        });

        if (contactId) {
          Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); // <--- VIBRATE
          Alert.alert("Success", "Saved to your phone contacts!");
        }
      } else {
        Alert.alert("Permission Denied", "Enable Contacts permission in settings to save this person.");
      }
    } catch (err) {
      console.log(err);
  ...
  const openLink = async (url) => {
    try {
      ...
      else Alert.alert("Error", "Cannot open this link: " + url);
    } catch (err) {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.card}>
          ...
          <TouchableOpacity style={styles.saveContactButton} onPress={saveToPhoneContacts}>
            ...
            <Text style={styles.saveContactText}>Save to Phone</Text>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  role: { color: COLORS.secondary, fontSize: 18, textAlign: 'center', marginBottom: 15 },
  ...
  saveContactText: { color: 'black', fontWeight: 'bold', marginLeft: 8 },

  socialContainer: { flexDirection: 'row', gap: 25, marginTop: 5, marginBottom: 5 },
